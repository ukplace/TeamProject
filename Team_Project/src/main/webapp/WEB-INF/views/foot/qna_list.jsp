		...
		<div class="container">
			<div class="board_list_wrap">
								...
								<tr>
									...
									<c:choose>
										<c:when test="${qnaDTO.qna_type eq 1 }">
											<td><a
												href="${pageContext.request.contextPath}/center/qna_detail?qna_idx=${qnaDTO.qna_idx}&page=${pageDTO.pageNum }"
												class="tit">주문내역/배송현황</a></td>
										</c:when>
										<c:when test="${qnaDTO.qna_type eq 2 }">
											<td><a
												href="${pageContext.request.contextPath}/center/qna_detail?qna_idx=${qnaDTO.qna_idx}&page=${pageDTO.pageNum }"
												class="tit">주문상품 취소하기</a></td>
										</c:when>
										<c:when test="${qnaDTO.qna_type eq 3 }">
											<td><a
												href="${pageContext.request.contextPath}/center/qna_detail?qna_idx=${qnaDTO.qna_idx}&page=${pageDTO.pageNum }"
												class="tit">반품/교환/AS 신청하기</a></td>
										</c:when>
										<c:when test="${qnaDTO.qna_type eq 4 }">
											<td><a
												href="${pageContext.request.contextPath}/center/qna_detail?qna_idx=${qnaDTO.qna_idx}&page=${pageDTO.pageNum }"
												class="tit">아이디/비밀번호찾기</a></td>
										</c:when>
										<c:when test="${qnaDTO.qna_type eq 5 }">
											<td><a
												href="${pageContext.request.contextPath}/center/qna_detail?qna_idx=${qnaDTO.qna_idx}&page=${pageDTO.pageNum }"
												class="tit">포인트 사용방법</a></td>
										</c:when>
										<c:otherwise>
											<td style="text-decoration: underline; font-weight: bold;"><a
												href="${pageContext.request.contextPath}/center/qna_detail?qna_idx=${qnaDTO.qna_idx}&page=${pageDTO.pageNum }"
												class="tit"> SHUSHU에서 답변드립니다.</a></td>
										</c:otherwise>
									</c:choose>

									<td><a
										href="${pageContext.request.contextPath}/center/qna_detail?qna_idx=${qnaDTO.qna_idx}&page=${pageDTO.pageNum } "
										class="tit"> ${qnaDTO.qna_subject}</a></td>
									<td><a
										href="${pageContext.request.contextPath}/center/qna_detail?qna_idx=${qnaDTO.qna_idx}&page=${pageDTO.pageNum } "
										class="tit">${qnaDTO.qna_name}</a></td>
									<td><fmt:formatDate value="${qnaDTO.qna_date}"
				...
				<br> <input type="button"
					onclick="location.href='${pageContext.request.contextPath}/center/qna_write'"
					class="qna_insert" value="글쓰기"></span>
				<!-- 페이징 -->
				<div class="row">
							...
							<ul>
								<li class="active"><c:choose>
										<c:when test="${pageDTO.startPage > pageDTO.pageBlock }">
											<a
												href="${pageContext.request.contextPath}/center/qna_list?pageNum=${pageDTO.startPage-pageDTO.pageBlock}"><i
								...
								<li class="active"><c:forEach var="i"
										...
										step="1">
										<a
											href="${pageContext.request.contextPath}/center/qna_list?pageNum=${i }"
											${1 }>${i }</a>
									</c:forEach></li>
								<!-- 				                  <li><a href="#">2</a></li> -->
								<!-- 				                  <li><a href="#">3</a></li> -->
								...
								<li class="active">
									<!-- end페이지가 count보다 더 클때 --> <c:choose>
										<c:when test="${pageDTO.endPage > pageDTO.pageCount }">
											<a
												href="${pageContext.request.contextPath}/center/qna_list?pageNum=${pageDTO.startPage+pageDTO.pageBlock}">다음<i
